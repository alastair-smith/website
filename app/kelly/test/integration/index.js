...
describe('Integration tests', () => {
  ...
  it('should return a response from the api gateway', async () => {
    ...
    const apiEndpoint = (await apiGateway.getApis({}).promise()).Items.find(({ Name: name }) => name === apiName).ApiEndpoint

    const result = await axios.get(`${apiEndpoint}/kelly-lambda-stage/?text=hello`)
